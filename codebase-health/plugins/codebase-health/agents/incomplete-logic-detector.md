...
- **LOG. Logging & observability anti-patterns** — stdout/stderr as production log channel (debug prints LOW, library/server paths MED), log-and-swallow (Category B's stricter cousin: the log line makes it LOOK handled), sensitive values in logs (a security lens — precedence routes it to `security/logging`), absence of structured logging/correlation IDs (module-level, hard-capped MED needs-verification per the 1.4.0 absence gate in the severity rubric).
