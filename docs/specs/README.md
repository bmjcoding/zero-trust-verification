...
Four genres live here; read each accordingly.
...
- **Product-spec-in-flight** — [`product-platform-spec.md`](product-platform-spec.md) + [`product-platform-spec.manifest.yaml`](product-platform-spec.manifest.yaml) + the two `docs/adr/DRAFT-product-platform-spec-*` ADRs: a product spec awaiting its build (a candidate for a `gateway/` subtree so the suite's own contracts stand out).
