...
- **Shared mock/test helpers:** when the Subtask touched a test-tree module imported by tests beyond the changed dirs (fakes, fixture factories, conftest-registered helpers), `{paths}` = ALL test files importing the touched helper(s) — repo-wide import scan, not just the touched files. This holds even for a single-edit Subtask; a repo-wide helper regression escapes the touched-file net otherwise.
...
2. **Validate STATUS.** Read `STATUS:` from the tracker frontmatter.
   ...
   - `ACTIVE` → inspect the session lock before refusing (**stale-ACTIVE reclaim**):
     ...
     - `session_lock` null or expired → the owning session died mid-drain without reaching a terminal STATUS (the classic case: between D7.5 and D8 while `awaiting_ci: true` — see D8 §Terminal-fire contract). Flip `STATUS: ACTIVE → PAUSED` with `status_reason: "session_ended_between_ci_polls"` when `in_progress.awaiting_ci: true` (otherwise `status_reason: "session_ended_mid_fire"`), append a `## Drift Notes` entry recording the reclaim, and continue to the drift check below as the normal PAUSED path.
