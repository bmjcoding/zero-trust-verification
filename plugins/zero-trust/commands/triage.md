...
invariants, the six-step run (probe → window → loop-guard → correlate →
emit → hand off, each a deterministic script call), and the judgment
boundary.
...
manifest / empty window): say so, escalate the gap, and do not fabricate a
join.
