...
**Manifest union (MS §2 / AV3-03), multi-doc only.** `bash ${SKILL_DIR}/scripts/validate_manifest_union.sh --union <a.manifest.yaml> <b.manifest.yaml> ...`: a Journey/Behavior ID shared across manifests → `[GENERATE-FAILED: manifest-id-collision: <id>]` (interrogation-log `DL-###` IDs are per-manifest and not unioned); differing `environments` → `[GENERATE-FAILED: manifest-union-mismatch: environments]`.
