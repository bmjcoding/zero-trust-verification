...
8. **Question count follows the decision tree, not a budget.** The interview
   ...
   open items — exiting with the manifest `completeness: incomplete` is
   legal only if the human explicitly defers each open item (S6 rules
   unchanged).
9. **Grillable vs ungrillable.** A decision needing higher fidelity than
   ...
   propose the defer (the human's yes is the explicit defer S6 requires). Point at the re-entry door —
